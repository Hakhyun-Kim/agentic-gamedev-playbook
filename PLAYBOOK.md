...
> 서로 다른 장르·프레임워크의 게임 세 개에서 반복 검증된 "AI 페어 게임 개발" 방식을
...
**장르도 프레임워크도 다르지만 아래 방법론은 셋 모두에 동일하게 적용됐습니다.**
...
- **CI 스모크** — 부팅 시 콘솔 에러 0건 + 봇 사망 분포 정상 범위를 배포 게이트로.

---

## 10. 새 게임 부트스트랩 체크리스트 (복사해서 시작)
...
[ ] window.__game 디버그 훅 + ?rafshim + 고정 타임스텝
[ ] 게임필: 잔상 체력바·히트 플래시·콤보 팝·데미지 숫자
[ ] 진화/특수능력 "잭팟"(행동을 바꾸는 것) 최소 1종
[ ] 메타 진행(죽어도 남는 재화)
[ ] 자동 품질 조절(FPS 측정→강등)
[ ] 첫 실행 코치 칩 + 기록 카드 PNG
[ ] CLAUDE.md — 아키텍처·규칙·세계관 정답지·검증법
...
| 축 | 로그라이크(던전) | 디펜스(용사) | 공통 추상 |
|----|------------------|--------------|-----------|
| 코어 압박 | 층을 내려갈수록 강해지는 적 | 웨이브가 셀수록 강해지는 적 | 시간/진행에 따른 위협 증가 |
| 성장 | 드래프트로 아이템 획득 | 소환·조합·강화 | 무작위 + 결정 |
| 잭팟 | 진화 카드 | 전설 특수능력 | 행동을 바꾸는 순간 |
| 검증 | 사망 층 분포 | 도달 웨이브 분포 | 봇의 진행도 분포 |
| 실패 | 던전 리셋 | 성 함락 리셋 | 런 종료 → 메타 재화 |
