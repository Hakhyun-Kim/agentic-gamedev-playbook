...
만드는 모든 커밋 메시지 마지막 줄에 다음 트레일러를 넣어라:
${TRAILER}
${FORCE ? `\n⚠️ 이번 실행은 강제 재실행이다(--force). 위 트레일러가 붙은 커밋이 이미 있어도
그것을 이유로 중단하지 마라 — 멱등 판정은 실행기가 이미 끝냈다. 대신 문서를 읽어
이미 반영된 내용과 중복되는지를 확인하고, 중복뿐이면 그때 커밋 없이 끝내라.\n` : ''}
